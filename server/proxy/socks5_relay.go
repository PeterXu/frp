...
var groupNameRe = regexp.MustCompile(`^[a-zA-Z0-9_][a-zA-Z0-9_.-]{0,62}$`)
...
func (pxy *Socks5RelayServerProxy) Run() (remoteAddr string, err error) {
	...
	if group == "" {
		return "", fmt.Errorf("socks5_relay requires a non-empty group (set via client config 'group' or TLS certificate OU)")
	}
	if !groupNameRe.MatchString(group) {
		return "", fmt.Errorf("socks5_relay group name [%s] is invalid: must match %[2]s", group, groupNameRe.String())
	}
