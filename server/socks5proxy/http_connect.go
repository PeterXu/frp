...
const httpConnectHandshakeTimeout = 30 * time.Second
...
func (h *HTTPConnectHandler) handleConn(ctx context.Context, conn net.Conn) {
	...
	// Set deadline for request reading to prevent slowloris attacks
	conn.SetDeadline(time.Now().Add(httpConnectHandshakeTimeout))
	...
	// Clear deadline after reading request
	...
	// Safety net: ensure tracker entry is cleaned up even if we return early
	// before the main Remove defer is set up after wrapping.
	...
	for _, err := range errs {
		if err != nil {
			xl.Debugf("http connect relay error for %s: %v", conn.RemoteAddr(), err)
		}
	}
}
