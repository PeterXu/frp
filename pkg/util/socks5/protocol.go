...
// Package socks5 contains shared SOCKS5 protocol helpers.
// Keeping one source of truth prevents protocol drift.
package socks5
...
func Authenticate(conn net.Conn, method byte, expectedPassword string) (group, userID, targetUser string, err error) {
	...
	// Empty username is allowed: the caller may use a static fallback
	// when no routing info was supplied by the client.
	if len(username) == 0 {
		return "", "", "", nil
	}
