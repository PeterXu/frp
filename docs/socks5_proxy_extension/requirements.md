...
Group is a client-level setting, not a per-proxy setting:
- frpc config declares group via `ClientCommonConfig.Group` field
- TLS certificate OU field overrides the group if present
- Group is sent to frps in `loginMsg.Metas["group"]` during Login handshake
- One frpc instance belongs to exactly one group — multiple groups require separate frpc instances
...
# Group is a client-level setting (one frpc = one group)
group = "groupA"
